...
DP = DataProcessing(language=args.language, data_path=args.datapath, data_name=args.dataset, noise=args.noise)
...
for i in range(tot_layer):
    ...
    print(i)
    print(rp_log_data_list[i].shape)
    print(rp_question_data_list[i].shape)
    print(X.shape)
    print(y.shape)
    ...
    with open(os.path.join("results/predictions/", 'y_prob.txt'), 'a') as f:
        ...
        f.write(f"Model: {model_name}, Dataset: {args.dataset}, Language: {args.language}, ")
        f.write(f"Epoch {i} predictions:\n")
    ...
    with open(os.path.join("results/weights/opinion/", (args.dataset + "_" + model_name + "_" + str(i) + "_new.txt")), 'a') as file:
        # Write the parameters and accuracy in one line
        file.write(f"Model: {model_name}, Dataset: {args.dataset}, Language: {args.language}, Weights: {weights}\n")

    with open(os.path.join("results/weights/opinion/", (args.dataset + "_" + model_name + "_" + str(i) + "_new_weights.txt")), 'a') as file:
        # Write the parameters and accuracy in one line
        file.write(f"Model: {model_name}, Dataset: {args.dataset}, Language: {args.language}, Weights: {weights.shape}\n")
...
with open(os.path.join("results/acc/", "list_acc_9_9.txt"), 'a') as file:
    # Write the parameters and accuracy in one line
    file.write(f"Model: {model_name}, Dataset: {args.dataset}, Language: {args.language}, Accuracy List: {list_acc}\n")
# with open(os.path.join("results/", ("weights_"+args.dataset+"_"+model_name+".txt")), 'a') as file:
#     # Write the parameters and accuracy in one line
#     file.write(f"Model: {model_name}, Dataset: {args.dataset}, Language: {args.language}, Weights: {weights}\n")

data = list_acc
proportions = [i * 100 / (len(data) - 1) for i in range(len(data))]
# Plotting the data
plt.figure(figsize=(10, 6))
plt.plot(proportions, data, marker='o', color='red', linestyle='-', linewidth=2, markersize=6, label='Data Curve')
...
# plt.savefig("results/" + args.language + '/' + model_name + '_' + args.dataset + ".png")
